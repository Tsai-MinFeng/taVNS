...
// swt_mid is 1/10 of total wave length
...
void switching(){
  if(enb_switching){
    if(swt_ctr <= swt_pos){ //positive
      ...
      digitalWrite(12, LOW);
      swt_ctr ++;
    }else if(swt_ctr > swt_pos && swt_ctr <= swt_pos + swt_full/10){
      ...
      digitalWrite(12, HIGH);
      swt_ctr ++;
    }else if(swt_ctr > swt_pos + swt_full/10 && swt_ctr <= swt_pos + swt_full/10 + swt_neg){ //negative
      // if(swt_ctr == swt_pos + swt_full/10 + 1){
      //   enb_sendData_n = true;
      // }
      ...
      if(swt_ctr == swt_pos + swt_full/10 + swt_neg + 1){
        enb_sendData_p = true;
      }
      digitalWrite(11, LOW);
      digitalWrite(12, HIGH);
...
// data[0]         ,data[1]     ,data[2]  ,data[3]  ,(data[5],data[4]) ,data[6]      ,data[7]
// select Sti mode ,output mode ,pos amp. ,neg amp. ,total duration(T) ,pos dur.(T1) ,neg dur.(T2)
void dac_write_callback(uint16_t conn_hdl, BLECharacteristic* chr, uint8_t* data, uint16_t len)
{
  ...
  else if(data[0] == 0x01){ //tDCS DC mode
    enb_tDCSmode = true;
  }

  if(data[1] == 0x01){ //taVNS AC mode
    Serial.println("AC mode");
    enb_switching = true;
  }
  else if(data[1] == 0x02){ //taVNS DC mode
    Serial.println("DC mode");
    enb_switching = false;
    if(data[2] == 0x00){
      dac_data_p.myByte[0] = 0x00;
      dac_data_p.myByte[1] = data[2];
      enb_sendData_p = true;
    }
  }
  else if(data[1] == 0x03){
  ...
  if(data[2] != 0x00){           //pos amplitude
    dac_data_p.myByte[0] = 0x00;
    dac_data_p.myByte[1] = data[2];
    enb_sendData_p = true;
  }
  if(data[3] != 0x00){           //neg amplitude
    dac_data_n.myByte[0] = 0x00;
    dac_data_n.myByte[1] = data[3];
    enb_sendData_n = true;
  }
  ...
  swt_full_16.myByte[0] = data[4];
  swt_full_16.myByte[1] = data[5];
